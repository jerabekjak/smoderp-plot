...
# pomocna plot funkce
pp = function(fil,fil2,var,var2,yr1,yr2,xr,stejne){
  if (stejne){
...
delejyr <- function(fil,fil2,var,var2,stejne) {
  yr1 = NA
...
plot_  = function(fil,fil2,var,var2,ranges = NULL, stejne){
  ...
  if (is.null(ranges)){
    ...
    yr1 = out[[1]]
    yr2 = out[[2]]
    pp(fil,fil2,var,var2,yr1,yr2,xr,stejne)
  } else {
    xr = ranges$x
    if (stejne){
      yr1 = ranges$y
...
server <- function(input, output) {
  ...
  observeEvent(input$pngplot, { 
    ...
    if (is.null(ranges2$x)){
      plot_(input$soubor,input$soubor2,input$sloupec,input$sloupec2,NULL,input$somevalue)
    } else {
      plot_(input$soubor,input$soubor2,input$sloupec,input$sloupec2,ranges2,input$somevalue)
    }
    dev.off()
